...
- [x] API exposes `GET /llm/usage/today` returning `{ used_tokens, limit_tokens, ratio }` for the authenticated user. Wired in STORY-005 (2026-04-28). Returns 401 when no session; 200 with the running daily total + ratio against `LEARNPRO_DAILY_TOKEN_LIMIT` for authenticated users; `ratio: 0` when the limit is 0 (self-hosted unlimited mode) — no divide-by-zero leak. 3 endpoint tests in `apps/api/src/index.test.ts`.
- [x] When the budget is exceeded, the API responds 429 with `{ error: "daily_budget_exceeded", message: "..." }` rather than letting `TokenBudgetExceededError` leak as a 500. Wired in STORY-005 (2026-04-28) via Fastify `setErrorHandler`. The handler catches the error globally — any handler that calls into the `BudgetGatedLLMProvider` and hits the cap gets the friendly 429 instead of 500. 1 endpoint test exercises the path via a deliberately-failing test route.
- [ ] Manual smoke: with `LEARNPRO_DAILY_TOKEN_LIMIT=100` and a real Anthropic key, hitting the playground twice triggers the friendly message on call #2. **Deferred to STORY-011** — needs an end-to-end LLM-driven flow on the playground (currently the playground only runs sandboxed code, doesn't invoke the LLM). The 429 path itself is covered by the integration test above; the smoke test waits for a UI surface that calls the LLM.
...
- 2026-04-28 — STORY-005 mopped up 2 of the 3 deferred ACs: `GET /llm/usage/today` (with auth-gated 401) and the 429 mapping for `TokenBudgetExceededError`. Manual playground smoke remains deferred — it waits for an LLM-driven UI surface (STORY-011).
