...
- [x] Admin tool to inspect failed-gate variants — landed via [STORY-039e](STORY-039e-admin-failed-gate-surface.md) (new `variant_gate_failures` table + read-only `GET /v1/admin/variant-failures` Fastify route gated on `users.is_admin = true` + minimal Next.js page at `/admin/variant-failures`).
...
- 2026-05-11 — STORY-039e mopped up AC #7 (admin tool to inspect failed-gate variants). New persistent `variant_gate_failures` table (migration 0024) + `users.is_admin` flag (migration 0025) + agent `failureLogger` callback + read-only `GET /v1/admin/variant-failures` Fastify route + minimal Next.js admin page at `/admin/variant-failures`. Failed-gate attempts (parse / identity / self-validation / retry exhaustion) are now persisted with structured payloads instead of dropped silently.
