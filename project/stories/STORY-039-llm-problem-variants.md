...
- [x] LLM-judge spec-clarity rubric — landed via [STORY-039d](STORY-039d-variant-spec-clarity-judge.md) on 2026-05-11. New `runVariantSpecClarityJudge` Haiku agent scores variants 1-5 on instruction_clarity / example_quality / concept_match. Variants with `min(scores) < 3` are dropped via `variants_dropped_spec_clarity` telemetry. Behind `LEARNPRO_VARIANT_SPEC_CLARITY_JUDGE=1` env flag (default ON with API key).
...
- 2026-05-11 — LLM-judge spec-clarity rubric AC landed via [STORY-039d](STORY-039d-variant-spec-clarity-judge.md). Adds a second Haiku call after the structural Zod gate that scores variants 1-5 on `instruction_clarity` / `example_quality` / `concept_match`; `min(scores) < 3` drops the variant. Behind `LEARNPRO_VARIANT_SPEC_CLARITY_JUDGE=1` env flag (default ON when ANTHROPIC_API_KEY is set). 54 new tests across prompt (12) + judge (25) + agent wiring (9) + route wiring (8).
