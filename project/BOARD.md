...
> **Last updated:** 2026-05-01 (STORY-019 done. New `@learnpro/tracks` workspace package mirrors `@learnpro/problems`: Zod-validated `TrackSchema` + `ConceptCardSchema` (kebab slugs, prerequisite chain, problem-slug refs); `python-fundamentals.yaml` covers 9 ordered concepts (variables-and-types → control-flow → strings → lists-and-tuples → dicts-and-sets → comprehensions → functions-and-closures → classes-basics → errors-and-exceptions), each with a 40+-char tutor-quotable summary and 1-3 problem refs into the STORY-016 bank (21 refs covering 19 of 33 Python problems). 3 spec concepts (`file-io`, `modules-and-packages`, `typing-basics`) deferred — bank lacks coverage; loader's "no orphan refs at parse time" invariant would reject them. `loadTrack(yaml)` rejects orphan problem refs / forward prerequisites / duplicate concept slugs at parse time; `seedTrack(db, track)` is idempotent under `(org_id, slug)` for tracks and `(org_id, language, slug)` for concepts. Concept-to-problem mapping stays on YAML rather than a join table — 33 problems is small enough that read-time YAML lookup beats migration surface in MVP. 35 unit tests (21 schema + 14 loader) + 1 DATABASE_URL-gated integration test. AC #4 (track picker in onboarding) and AC #5 (progress bar UI) deferred to the as-yet-unfiled `/dashboard` Story and STORY-022 respectively. STORY-053 + STORY-016 both done. STORY-053: conversational onboarding agent ships end-to-end. New `POST /v1/onboarding/turn` Fastify endpoint orchestrates a multi-turn warm-coach chat using Anthropic Haiku via the existing `LLMProvider`; system prompt embeds the JSON output schema for structural per-turn extraction. `updateProfileFields()` in `@learnpro/db` does a partial UPSERT — only supplied non-null columns write. `apps/web/src/app/onboarding/page.tsx` replaces the STORY-005 placeholder with a full chat UI (assistant/user bubbles, "Step N / 6" indicator, "Start now (skip)" link, friendly 429/503 banner, brief delay-then-route on `done=true`). `MAX_ONBOARDING_TURNS=6` + `MAX_ONBOARDING_TOKENS=3000` caps; `LEARNPRO_DISABLE_ONBOARDING_LLM=1` switches to a deterministic 3-question state machine — guarantees AC #6 onboarding-never-blocks-signin. STORY-016: curated seed problem bank in `packages/problems/` with 33 Python + 30 TypeScript YAMLs; per-language distribution hits the spec target (10 L1-2 / 15 L3 / 5 L4-5 for TS; 13/17/3 for Python within tolerance). `ProblemDefSchema` enforces kebab-case slugs + concept tags + difficulty 1-5 + non-empty hidden tests; `seedProblems(db, defs)` idempotent under `(org_id, track_id, slug)`; `validateProblems(defs, sandbox)` iterates each problem via a per-language `__LEARNPRO_PASS__` / `__LEARNPRO_FAIL__{json}` verdict harness. 35 problem-bank tests + 64 onboarding tests = 99 new this session. **STORY-011 (tutor agent) is now unblocked** — that closes the MVP loop.)
