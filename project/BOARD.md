...
> **Last updated:** 2026-05-06 (STORY-035 + STORY-031 both done. **MVP P0 backlog still empty; v1 P0 also closing fast.** STORY-035: prompt eval harness with 50 canned student transcripts (15 hint + 15 grade + 10 onboarding + 10 session-plan), two-layer scoring (deterministic regex + Haiku-as-judge per behavior tag), `pnpm --filter @learnpro/agent eval` CLI with `--filter`/`--baseline`/`--markdown-out`, prompt-eval CI workflow on PRs touching prompts or evals. 25 new tests. STORY-031: FSRS spaced-repetition scheduler — pure `ts-fsrs@5.3.2` wrapper, `concept_reviews` table (migration `0011_concept_reviews.sql`), `updateProfile` writes FSRS row per concept tag, `assignProblem` breaks ties toward due-concept problems (secondary signal), `GET /v1/spaced-repetition/due` + `<DueReviewsCard>` on dashboard. ~58 new tests. Earlier (2026-05-05): STORY-054 + STORY-021.)
...
Path A locked 2026-04-25. EPIC-019 (foundation) + auth (STORY-005) + onboarding (STORY-053) + seed bank (STORY-016) + tutor agent (STORY-011) + tutor session UI (STORY-062) + 3-rung hint ladder (STORY-017) all shipped. **The MVP single learning loop is end-to-end through the UI.** STORY-021 (career-aware onboarding interview + Recommended-for-you page) just landed — first-login users now route to `/recommended` instead of `/dashboard` when their `target_role` is set.
...
| [STORY-031](stories/STORY-031-fsrs-spaced-repetition.md) | FSRS spaced-repetition scheduler — pure `ts-fsrs@5.3.2` wrapper in `@learnpro/scoring` (`recomputeAfterReview` / `isDue` / `mapEpisodeOutcomeToGrade`); new `concept_reviews` table (migration `0011_concept_reviews.sql`) with unique (user_id, concept_id) for idempotent UPSERT; `getCardState` / `recordReview` / `getDueConcepts` (capped at 50) DB helpers; `updateProfile` writes one FSRS row per resolved concept tag; `assignProblem` breaks ties toward due-concept problems (secondary signal — never overrides heuristic or recency); new `GET /v1/spaced-repetition/due` Fastify route + Next.js proxy + coach-voice `<DueReviewsCard>` on the dashboard. ~58 new tests. | 2026-05-06 |
