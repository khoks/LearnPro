...
> **Last updated:** 2026-05-06 (STORY-045 done — Email digest channel (daily + weekly). New `packages/notifications/email/` with `EmailChannel`, REST-only `ResendTransport`, pure `buildDailyDigest()` / `buildWeeklyDigest()`. Migration `0014_email_digests.sql` adds 4 `profiles` columns. New Fastify routes: `/v1/settings/email-digest` GET/PUT, `/v1/email/unsubscribe` GET/POST (RFC 8058). Cron: extends `daily-reminder.ts` + new `weekly-digest.ts`. ~80 new tests. Earlier: STORY-059 done — sandbox SSE streaming; STORY-062 done — Redis rate limiter; STORY-061 done — importDump round-trip; STORY-040 done — GitHub portfolio; STORY-046 done — daily plan view; STORY-044 done — PWA baseline. MVP remains demoable; v1 in flight.)
