...
> **Last updated:** 2026-05-01 (STORY-054 done. Adaptive autonomy controller: new `profiles.confidence_signal jsonb` migration `0010_autonomy_signal.sql`. Pure `EwmaBandedAutonomyPolicy` in `@learnpro/scoring` — Low (<0.3) / Medium (0.3-0.7) / High (>0.7) bands; cold-start safety pins users to Low for the first 5 episodes. Tutor's `updateProfile.refreshConfidenceSignal` updates the signal on episode close (best-effort). Tutor's `TutorSession.consultAutonomy(kind)` branches at "should I just do this?" decision points (`assign-next-problem` / `proactive-hint` / `auto-set-final-outcome` / `switch-track`). Every decision writes an `autonomy_decision` event to `interactions` (STORY-055's existing event type, extended with optional `band` field). Replay fixture (`replay-002-autonomy.json`) exercises Low → Medium → High band progression over a 20-step transcript. New `<AutonomyBandIndicator>` on `/dashboard` with coach-voice tooltip + forbidden-phrase test. New `GET /v1/autonomy/state` route + Next.js proxy. ~60 new tests. Earlier: STORY-056 + STORY-024 + STORY-025 + STORY-015 + STORY-023 + STORY-027 + STORY-022 + STORY-062 + STORY-017 + STORY-020 all done.)
...
| ID | Title | Done |
|----|-------|------|
| [STORY-054](stories/STORY-054-adaptive-autonomy-controller.md) | Adaptive autonomy controller — new `profiles.confidence_signal jsonb` (migration `0010_autonomy_signal.sql`); pure `EwmaBandedAutonomyPolicy` in `@learnpro/scoring` with Low (<0.3) / Medium (0.3-0.7) / High (>0.7) bands and cold-start safety; `updateConfidenceSignal` EWMA helper; `getConfidenceSignal` / `updateConfidenceSignalRow` / `countClosedEpisodes` DB helpers. Tutor wires `refreshConfidenceSignal` into `updateProfile`'s deps and `TutorSession.consultAutonomy(kind)` covers four well-known action kinds (assign-next-problem / proactive-hint / auto-set-final-outcome / switch-track). Telemetry `onDecision` hook writes `autonomy_decision` events to `interactions` (STORY-055's schema extended with optional `band`). Replay fixture `replay-002-autonomy.json` covers a 20-step Low → Medium → High transcript. `GET /v1/autonomy/state` + Next.js proxy + `<AutonomyBandIndicator>` on `/dashboard` with coach-voice tooltip + forbidden-phrase test. ~60 new tests. | 2026-05-01 |
