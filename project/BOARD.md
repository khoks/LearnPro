...
> **Last updated:** 2026-05-06 (STORY-063 + STORY-035 + STORY-031 all done. **All 6 MVP done criteria satisfied — MVP is demoable.** STORY-063 closes MVP done criterion #5: vitest fetch-driver e2e suite drives sign-in → onboarding (deterministic fallback) → recommended → session → submit-passing → finish → next-problem against real Fastify + real Postgres + Drizzle tutor factory (gated on `LEARNPRO_E2E=1`). Asserts ≥5 DB row deltas. New `pnpm e2e` script boots a dedicated Postgres container on port 5433. Migration 0008 made idempotent. STORY-035: prompt eval harness — 50 transcripts, two-layer scoring (regex + Haiku-as-judge), `pnpm eval` CLI, prompt-eval CI workflow. 25 new tests. STORY-031: FSRS spaced-repetition — `ts-fsrs@5.3.2` wrapper, `concept_reviews` table (migration `0011`), tutor integration as secondary signal, `<DueReviewsCard>` on dashboard. ~58 new tests. Earlier (2026-05-05): STORY-054 + STORY-021.)
