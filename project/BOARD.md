...
> **Last updated:** 2026-04-28 (STORY-010 done — every ADR-0002 hardening checklist item now has an automated breakout test under `packages/sandbox/test/breakout/`. 13 test files + a shared `runBreakout()` harness with two modes: structural (default, runs in CI against a `HardenedStubSandboxProvider`) and live-Piston (opt-in via `LEARNPRO_REQUIRE_PISTON=1`). 38 tests pass; 4 live-only assertions skip without the env. Hardened `infra/docker/docker-compose.dev.yaml` while at it: non-root UID 1000:1000, sized tmpfs `/tmp`, `cap_drop: [ALL]`, `no-new-privileges`, pids/mem/cpu container-level fences. AC-4 enforced as a real CI test that ripgreps the repo for `--privileged` outside an allowlist.)
