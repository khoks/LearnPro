...
> **Last updated:** 2026-05-01 (STORY-035 done — first v1 P0 to land. Prompt eval harness with 50 canned student transcripts (15 hint + 15 grade + 10 onboarding + 10 session-plan), two-layer scoring (deterministic regex/JSON-shape + Haiku-as-judge per behavior tag), `pnpm --filter @learnpro/agent eval` CLI with `--filter` / `--baseline` / `--markdown-out`, prompt-eval CI workflow on PRs touching prompts or evals (posts markdown PR comment, fails on regressions vs main's baseline). 25 new tests on the harness itself. Earlier (2026-05-05): STORY-054 + STORY-021 closed; **MVP P0 backlog is empty**.)
...
| ID | Title | Done |
|----|-------|------|
| [STORY-035](stories/STORY-035-prompt-eval-harness.md) | Prompt eval harness — 50 canned student transcripts under `packages/agent/evals/transcripts/` (15 hint + 15 grade + 10 onboarding + 10 session-plan); `runEvals()` two-layer scorer (deterministic `explicit_failure_patterns` regex sweep + JSON-shape gate, then Haiku-as-judge per `expected_behavior_tag` with `{ passed, reasoning }` rubric); `EvalReport` Zod schema + per-category + per-tag aggregates + `diffReports()`; `pnpm --filter @learnpro/agent eval` CLI with `--filter` / `--baseline` / `--markdown-out`; `.github/workflows/prompt-eval.yml` triggers on PRs touching `packages/prompts/src/**` or `packages/agent/evals/**`, posts a markdown summary as a PR comment, fails on regressions vs main's most-recent committed report. Cost ~$0.50–$2 per full run (Haiku). 25 new tests on the harness itself. | 2026-05-01 |
