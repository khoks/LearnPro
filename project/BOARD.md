...
> **Last updated:** 2026-05-05 (STORY-054 + STORY-021 both done. **MVP P0 backlog is empty.** STORY-054: adaptive autonomy controller — `profiles.confidence_signal jsonb` (migration `0010_autonomy_signal.sql`), pure `EwmaBandedAutonomyPolicy` in `@learnpro/scoring` (3 bands, cold-start safety pins users to Low for first 5 episodes), `updateConfidenceSignal` EWMA helper, `TutorSession.consultAutonomy(kind)` branches at 4 decision points, `autonomy_decision` events written to `interactions`, replay fixture covers Low→Medium→High over 20 steps, `GET /v1/autonomy/state` + `<AutonomyBandIndicator>` on /dashboard. ~60 new tests. STORY-021: career-aware onboarding — new `@learnpro/profile` package with 7 hardcoded MVP roles + `getRecommendation()`, `GET /v1/recommendation` joins role's `recommended_track_slugs` against `tracks` table, `/recommended` page renders post-onboarding with bias-specific hints + clickable track cards + free-choice "Take me to the dashboard" link, `destinationFor()` post-signin rule routes target_role+0-episodes users to /recommended. 54 new tests. Earlier today: STORY-056 + STORY-024 + STORY-025 + STORY-015 + STORY-023 + STORY-027 + STORY-022 + STORY-062 + STORY-017 + STORY-020 all done.)
