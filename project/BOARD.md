...
> **Last updated:** 2026-05-01 (STORY-025 done — responsive web baseline. Path A: Tailwind isn't installed in MVP; the equivalent guarantee for AC #4 is a new `useViewportSize()` hook + `BREAKPOINTS` constants in `apps/web/src/lib/`. `/dashboard` cards stack <768; `/session` sidebar collapses into a "Show plan" drawer <1024 (AC #3); `/playground` controls stack <768; `/onboarding` chat bubbles capped at `min(100%, 600px)`. Outer `<main>` padding tightened to 1.25rem on every page so 320–768 widths fit without overflow. 58 new tests, 291 passing in apps/web. Earlier: STORY-015 + STORY-023 + STORY-027 + STORY-022 + STORY-062 + STORY-017 + STORY-020 all done.)
