...
> **Last updated:** 2026-05-06 (STORY-061 done — `importDump()` companion to STORY-026's exporter. New `packages/db/src/data-import.ts` accepts the JSON envelope `exportUserData()` produces, runs the whole import inside a single transaction (no half-imported users on partial failure), Zod-validates the envelope at the entry point, creates the user if absent, UPSERTs profile, inserts episodes/submissions/agent_calls/notifications preserving original UUIDs. Idempotent — UUID collisions skip with a clear warning and the result envelope reports them under `skipped`. `embedding` on episodes intentionally omitted (re-derivable). Problems are not in the export envelope; pre-flight check fails fast if a referenced `problem_id` is missing on the target instance. New CLI at `packages/db/bin/import.ts`, wired as `pnpm --filter @learnpro/db db:import < dump.json` (stdout = result JSON; stderr = warnings). 16 new tests (3 schema-validation + 13 integration). Earlier: STORY-040 done — `learnpro-portfolio` GitHub auto-push (new `@learnpro/portfolio` workspace package + 5 API routes + 6 Next.js proxies + `<PortfolioCard>` + `<SaveToPortfolioButton>`); STORY-046 done — daily plan view (weekly deferred to STORY-046b); STORY-044 done — PWA baseline. Earlier: 2026-05-06 closed all 6 MVP done criteria (STORY-063 + STORY-035 + STORY-031). MVP remains demoable; v1 in flight.)
...
| ID | Title | Done |
|----|-------|------|
| [STORY-061](stories/STORY-061-import-dump.md) | `importDump()` companion to STORY-026 — round-trip a JSON export back into a fresh instance. New `packages/db/src/data-import.ts` (Zod-validated envelope; transaction-wrapped; preserves original UUIDs; UUID collisions skip with a warning rather than overwrite). Pre-flight check fails fast if any referenced `problem_id` is missing on the target instance (the export envelope intentionally omits the problems catalog — sane for self-hosted migrations). `embedding` on episodes intentionally omitted on both sides (re-derivable). New CLI at `packages/db/bin/import.ts`, wired as `pnpm --filter @learnpro/db db:import < dump.json`. 16 new tests (3 no-DB Zod-schema + 13 integration covering user create / existing-user identity preservation / profile UPSERT insert+update / null-profile no-op / row-id preservation / FK pre-flight failure / round-trip / idempotency / collision warning / warnings array). | 2026-05-06 |
