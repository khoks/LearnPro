...
> **Last updated:** 2026-05-06 (STORY-042 done — Anti-cheat v1: paste-detect modal + per-submission "I got help" toggle + got-help-aware skill skip. Migration 0015 adds `episodes.got_help boolean default false NOT NULL`. New `<PasteDetectModal>` (role=dialog, ESC dismisses, single-fire-per-paste) wired into PlaygroundClient + SessionClient via shared `attachPasteListener` (Monaco onDidPaste + DOM-paste fallback). Per-submission honesty toggle in the session result panel POSTs to `/api/tutor/episodes/:id/got-help`. Close-time skill skip via `wrapWithGotHelpAwareSkillSkip` decorator wrapping `buildUpdateProfileDrizzleDeps`. New `assign-problem-v3` prompt with `previous_got_help` walk-through phrasing. New `<HonestSessionsCard>` on the dashboard with coach-voice copy. Data export/import round-trip the column. ~50 new tests. Earlier: STORY-045 done — Email digest channel (daily + weekly); STORY-059 done — sandbox SSE streaming; STORY-062 done — Redis rate limiter; STORY-061 done — importDump round-trip; STORY-040 done — GitHub portfolio; STORY-046 done — daily plan view; STORY-044 done — PWA baseline. MVP remains demoable; v1 in flight.)
