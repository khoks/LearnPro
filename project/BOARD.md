...
> **Last updated:** 2026-05-04 (STORY-024 + STORY-025 both done. STORY-024: user-configurable quiet hours land via 4 new `profiles` columns + new `deferred_notifications` table (migration `0009_quiet_hours.sql`), pure DST-aware `isInQuietHours` + `nextDeliveryTime` policy in `@learnpro/scoring`, `QuietHoursDispatcher` factory composing the existing `NotificationDispatcher`, `processDeferredNotifications` flusher (LIMIT 100, idempotent), 2 settings routes + Next.js proxies + `<QuietHoursCard>` UI on the dashboard. Daily-reminder cron now uses the wrapped dispatcher. **Notifications deferred, never dropped** (AC #4). ~66 new tests. STORY-025: responsive web baseline. Path A: Tailwind isn't installed in MVP; the equivalent guarantee for AC #4 is a new `useViewportSize()` hook + `BREAKPOINTS` constants in `apps/web/src/lib/`. `/dashboard` cards stack <768; `/session` sidebar collapses into a "Show plan" drawer <1024 (AC #3); `/playground` controls stack <768; `/onboarding` chat bubbles capped at `min(100%, 600px)`. Outer `<main>` padding tightened to 1.25rem on every page. 58 new tests. Earlier: STORY-015 + STORY-023 + STORY-027 + STORY-022 + STORY-062 + STORY-017 + STORY-020 all done.)
