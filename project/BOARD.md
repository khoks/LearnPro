...
> **Last updated:** 2026-05-04 (STORY-015 + STORY-023 + STORY-027 + STORY-022 + STORY-062 + STORY-017 + STORY-020 all done. STORY-015: session-plan agent end-to-end — new `session_plans` table (migration `0007_session_plans.sql` — renumbered from 0006 to settle the collision with STORY-023's notifications migration), `planSession` Haiku tool with deterministic 3-item fallback, 3 Fastify routes, tutor auto-marks on episode close, `<SessionPlanSidebar>` in `/session`. STORY-023: bell icon + Web Push + daily-reminder cron via new `@learnpro/notifications` package + `web_push_subscriptions` table + `notifications.dedupe_key` (migration `0006_notifications_web_push.sql`); STORY-024 unblocked via `dispatcher.shouldDeliverNow` seam. STORY-027: a11y baseline — global skip link, `:focus-visible` double-ring, Monaco `role="region"`, new `<StatusBadge>`, `axe-core` 0-critical sweep across 6 pages. STORY-022: XP / streak (UTC-day, monthly grace, "graces don't dangle") + `xp_awards` table + real `/dashboard`. STORY-062: full `/session` page wires 4 tutor routes via state machine + driver + Monaco. STORY-017: single Hint button escalating rung 1 → 2 → 3. STORY-020: TS fundamentals track (12 of 13 concepts, modules deferred). Friendly 401/403/404/409/429/502/503 banners across all UIs. ~230 new tests across the seven Stories.)
...
|----|-------|------|-------|----------|-----|
| [STORY-021](stories/STORY-021-onboarding-interview.md) | Career-aware onboarding interview (target role, time budget, level) | EPIC-010 | mvp | P0 | S |
| [STORY-024](stories/STORY-024-quiet-hours.md) | User-configurable quiet hours (now unblocked — wires into `dispatcher.shouldDeliverNow`) | EPIC-012 | mvp | P1 | XS |
