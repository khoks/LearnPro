...
> **Last updated:** 2026-05-03 (STORY-023 + STORY-027 + STORY-022 + STORY-062 + STORY-017 + STORY-020 all done. STORY-023: bell icon + Web Push + daily-reminder cron via new `@learnpro/notifications` package. New `web_push_subscriptions` table + `notifications.dedupe_key` (migration `0006_notifications_web_push.sql`). 6 Fastify routes; service worker + dropdown panel in `apps/web`; coach-voice copy with forbidden-phrase test. STORY-024 (quiet hours) is unblocked via `dispatcher.shouldDeliverNow` seam. STORY-027: a11y baseline — global skip link, `:focus-visible` double-ring, Monaco `role="region"` + `aria-describedby`, new `<StatusBadge>` (icon + text, never colour-only), `axe-core` sweep across 6 pages with 0-critical-violation gate. STORY-022: XP / streak (UTC-day, monthly grace, "graces don't dangle") + `xp_awards` table + real `/dashboard` page. STORY-062: full `/session` page wires 4 tutor routes via state machine + driver + Monaco. STORY-017: single Hint button escalating rung 1 → 2 → 3 with API-supplied XP costs. STORY-020: TS fundamentals track (12 of 13 concepts, modules deferred). Friendly 401/403/404/409/429/502/503 banners across all UIs. ~160 new tests across the six Stories.)
