...
> **Last updated:** 2026-05-03 (STORY-026 + STORY-019 + STORY-011 all done. **MVP loop is API-complete.** STORY-011: hand-rolled tutor agent harness in new `@learnpro/agent` (no LangChain — per ADR-0003) with state machine `assign → coding → (hint | submit) → grading → profile-update → next` + 4 tools (`assignProblem` / `giveHint` / `grade` / `updateProfile`); 4 Fastify routes (`/v1/tutor/episodes/...`); replay-fixture eval test + DATABASE_URL-gated integration. UI deferred to STORY-062 (filed). STORY-026: GDPR-style `GET /v1/export` streams JSON envelope (profile / episodes / submissions / agent_calls / notifications / settings) scoped by session cookie; in-memory `MemoryRateLimiter` 1/hr per user; round-trip-importable shape; `importDump()` → STORY-061, Redis limiter → STORY-062 (existing). STORY-019: new `@learnpro/tracks` package mirrors `@learnpro/problems`; `python-fundamentals.yaml` covers 9 ordered concepts (3 spec concepts deferred until seed bank covers them) with 21 refs into 19 of 33 Python problems; loader rejects orphan refs / forward prerequisites at parse time. ~125 new tests across the three Stories; **426 passing / 32 skipped repo-wide**. The MVP single-loop is now end-to-end functional from API: pick problem → run sandbox → grade → update profile → next.)
