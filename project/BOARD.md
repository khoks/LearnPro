...
> **Last updated:** 2026-05-03 (STORY-027 a11y baseline done — global skip link → `#main-content`, double-ring `:focus-visible` styling, Monaco wrapped in `role="region"` + `aria-describedby` keyboard-help paragraph, new `<StatusBadge>` component conveys pass/fail via icon + text, `axe-core` sweep across 6 top-level pages with 0-critical-violation gate. STORY-022 + STORY-062 + STORY-017 + STORY-020 all done. **MVP loop is end-to-end through the UI with humane gamification.** STORY-022: schema migration `0005_xp_streak.sql` adds `users.xp` + grace counters + `xp_awards` table (idempotent via `(user_id, episode_id, reason)` unique). Pure `awardXpForEpisode` (hint-cost ladder `[5,15,30]` per STORY-017) + `computeStreak` (UTC-day, monthly-replenishing grace, "graces don't dangle" semantics) in `@learnpro/scoring`. Tutor's `updateProfile` calls `awardXp` on close. Real `/dashboard` page replaces the placeholder — `XpCard` + `StreakCard` + `TrackProgressBar` + "Start a session" CTA; coach-voice copy explicitly tested for absence of "DON'T LOSE" / "DAY X" / "burn" / 🔥 / ⚠️. STORY-062: new `/session?track=...` page wires the 4 tutor API routes via pure state machine + driver + 4 Next.js proxies + Monaco editor; full assign → submit → finish flow tested without jsdom+RTL. STORY-017: single Hint button escalates rung 1 → 2 → 3, disabled after rung 3; `hint_request`/`hint_received` events flow into STORY-055's interactions table. STORY-020: TS fundamentals track — 12 of 13 concepts (modules deferred). Friendly inline banners for 401/403/404/409/429/502/503 — never blank-screens. ~140 new tests across the four Stories.)
