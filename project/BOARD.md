...
> **Last updated:** 2026-05-04 (STORY-056 + STORY-024 + STORY-025 all done. STORY-056: pure regex `redactPii` for 5 PII categories (email / phone / url / Luhn-checked credit_card / gov_id) in `@learnpro/shared`; new `@learnpro/redaction` package with Haiku second-pass + `NoopLlmRedactor` + `OrchestratedRedactor`. 3 retention sweepers in `packages/db/src/retention.ts` (raw-LLM 90d no-op pending raw-text columns; voice 30d delete; interactions 90d aggregate-then-delete) + `pnpm --filter @learnpro/db db:retention` script. Wired into `/v1/interactions` (voice), `/v1/onboarding/turn`, `/v1/tutor/episodes/:id/submit`. New `GET /v1/data/summary` + `DELETE /v1/data/voice` + `DELETE /v1/data/account` + `/settings/data` page. `docs/security/RETENTION.md` documents windows + patterns. ~80 new tests. STORY-024: user-configurable quiet hours via 4 new `profiles` columns + `deferred_notifications` table (migration `0009_quiet_hours.sql`), pure DST-aware policy in `@learnpro/scoring`, `QuietHoursDispatcher` factory + `processDeferredNotifications` flusher; 2 settings routes + `<QuietHoursCard>` UI; daily-reminder cron uses wrapped dispatcher. **Notifications deferred, never dropped.** ~66 new tests. STORY-025: responsive web baseline (Path A: no Tailwind, just `useViewportSize` + `BREAKPOINTS`). `/dashboard` cards stack <768; `/session` sidebar collapses into a "Show plan" drawer <1024; `/playground` controls stack <768. 58 new tests. Earlier: STORY-015 + STORY-023 + STORY-027 + STORY-022 + STORY-062 + STORY-017 + STORY-020 all done.)
