...
> **Last updated:** 2026-05-03 (STORY-022 done — minimal gamification surface complete. **Schema:** `users.xp` + `users.streak_grace_days_remaining` + `users.streak_grace_last_replenished_at` + new `xp_awards` table (migration `0005_xp_streak.sql`) with `(user_id, episode_id, reason)` unique constraint for idempotent awards. **Policies (pure, in `@learnpro/scoring`):** `awardXpForEpisode` (configurable XP table, hint-cost ladder `[5, 15, 30]` per STORY-017) + `computeStreak` (UTC-day step, monthly-replenishing grace, "graces don't dangle" semantics — they only commit when bridging to a real episode day, so the streak number is never inflated by unused grace credit). **DB helpers in `@learnpro/db/xp-streak.ts`:** idempotent `awardXp` (ON CONFLICT DO NOTHING + conditional `users.xp` increment), lazy monthly `replenishGraceDays`, `getStreakInputs`/`getStreakSnapshot` (90-day lookback), `getActiveTrackSlugs`, `getTrackProgress` (concept-tag count vs `skill_scores.confidence >= 50`). **Tutor:** `updateProfile` tool now calls `awardXp` on close — surfaces `xp_award` in its output even for failed/abandoned (amount=0) so the dashboard's "you tried N problems" history is complete. **Dashboard:** real `/dashboard` page replaces the placeholder; 3 pure components (`XpCard` / `StreakCard` / `TrackProgressBar`); coach-voice copy explicitly tested for absence of "DON'T LOSE" / "DAY X" / "burn" / 🔥 / ⚠️; "Start a session" CTA links to `/session?track=<active>`. ~50 new tests (8 xp-policy + 17 streak-policy + 7 schema + 18 db-integration + 7 tool + 13 component). The notification-copy AC is scoped to the dashboard for now — STORY-023 carries the rule forward when the notification system lands. **Previously (2026-05-03):** STORY-026 + STORY-019 + STORY-011 all done — MVP loop is API-complete with tutor agent + 4 tools + 4 Fastify routes + GDPR-style data export + Python fundamentals track.)
