...
> **Last updated:** 2026-05-06 (STORY-059 done — Live stdout/stderr streaming via Server-Sent Events. Extended `SandboxProvider` with `runStream(req, signal): AsyncIterable<SandboxRunChunk>`; v1 fake-streams by chunking the post-run output (Piston's HTTP API is request/response — Option 2 in the story) so STORY-010's 13 breakout tests still cover the streaming path verbatim. New `POST /v1/sandbox/run/stream` Fastify route emits one SSE event per chunk ending with `exit`; new Next.js Route Handler proxy + `runSandboxStream()` fetch + ReadableStream-reader helper; opt-in "Stream output" checkbox in `<PlaygroundClient>` (default off). 26 new tests. Earlier: STORY-062 done — Redis-backed `RateLimiter` for multi-process / multi-replica deployments. New `RedisRateLimiter` in `apps/api/src/rate-limiter.ts` using single-shot atomic `SET key value NX EX seconds` so two replicas can't both win the slot. Falls back to `MemoryRateLimiter` when `REDIS_URL` env isn't set (preserves dev path with no Redis). Integration tests gated by `LEARNPRO_REQUIRE_REDIS=1` against Docker Compose Redis (4 pass: 2-parallel race, 16-fanout race, two-user independence, real-Redis EXPIRE roundtrip). 16 new tests. STORY-061 done — `importDump()` companion to STORY-026's exporter (round-trip `exportUserData → importDump`, Zod-validated, transaction-wrapped, idempotent collision-skip). 16 new tests + CLI at `packages/db/bin/import.ts`. Earlier: STORY-040 done — `learnpro-portfolio` GitHub auto-push; STORY-046 done — daily plan view; STORY-044 done — PWA baseline. MVP remains demoable; v1 in flight.)
