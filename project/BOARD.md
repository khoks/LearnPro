...
> **Last updated:** 2026-05-06 (STORY-040 done — `learnpro-portfolio` GitHub auto-push. New `@learnpro/portfolio` workspace package with `GitHubPortfolioClient` (REST v3 + minimal scopes: `repo` only when auto-create requested, else `public_repo`) + Zod-validated readme template. New `accounts` row with `provider="github_portfolio"` + opt-in OAuth flow at `apps/web/src/portfolio/oauth.ts` (state-PKCE + 10-min state cookie + provider-side denial mapping). New 5 portfolio API routes (`/v1/portfolio/state` GET, `/connect-init` POST, `/disconnect` DELETE, `/push` POST, `/settings` PUT) + 6 Next.js proxies. New `<PortfolioCard>` on `/settings/portfolio` (connect/disconnect/repo-name/auto-push toggle/recent pushes log) + `<SaveToPortfolioButton>` on `/session` post-pass. Migration `0012_github_portfolio.sql` adds `portfolio_pushes` table + `profiles.portfolio_settings jsonb`. ~120 new tests. STORY-046 done — daily plan view (weekly deferred to STORY-046b). Pure read-time composer in `@learnpro/agent` (`packages/agent/src/today-plan.ts`) aggregates today's plan from session_plans + concept_reviews + episodes; re-plan dampening suppresses regenerate when only 1 weekday is missed or it's a weekend; coach-voice reasoning text per item. New `GET /v1/today-plan` + `POST /v1/today-plan/replan` Fastify routes + Next.js proxies + `/today` page UI. Earlier: STORY-044 done — first v1 Story to land post-MVP. PWA baseline: `apps/web/public/manifest.webmanifest` + 192/512/maskable SVG icons; layered offline cache onto STORY-023's existing `apps/web/public/sw.js` (preserved push handler verbatim) with cache-first for the shell, network-then-/offline for non-shell navigations, stale-while-revalidate for static assets, network-only for `/api/**`; pure routing in `apps/web/src/lib/sw-handlers.ts` so vitest can cover it. New `/offline` page (server component, coach-voice copy). New `<OfflineBanner>` on `/session` + `/playground` listening to `online`/`offline` events. New `GET /v1/dashboard/install-eligible` route + Next.js proxy: counts `passed | passed_with_hints` episodes, threshold = 3. New `<InstallPrompt>` on `/dashboard` listens to `beforeinstallprompt`, renders only when eligible AND not dismissed; localStorage key `learnpro:install-prompt-dismissed-at`. AC #7 (Lighthouse ≥ 90) scoped to operator-run audit per `apps/web/PWA-AUDIT.md` checklist (CI doesn't run Lighthouse — same scope-down pattern STORY-027 used). +29 new tests across `@learnpro/api`, `@learnpro/db`, `@learnpro/web`. Earlier: 2026-05-06 closed all 6 MVP done criteria (STORY-063 + STORY-035 + STORY-031). MVP remains demoable; v1 is now in flight.)
