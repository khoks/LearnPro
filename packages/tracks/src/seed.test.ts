...
import { loadTrack, PYTHON_FUNDAMENTALS_PATH, seedTrack } from "./loader.js";
...
describe.skipIf(!DATABASE_URL)("seedTrack (integration)", () => {
  let db: LearnProDb;
  ...
  afterAll(async () => {
    if (db) {
      await db
        .delete(concepts)
        .where(and(eq(concepts.org_id, SELF_HOSTED_ORG_ID), eq(concepts.language, "python")));
      await db
        .delete(tracks)
        .where(and(eq(tracks.org_id, SELF_HOSTED_ORG_ID), eq(tracks.slug, "python-fundamentals")));
    }
    await pool?.end();
  });
